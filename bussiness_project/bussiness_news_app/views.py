...
from .models import Articles, AboutAuthor
from .forms import UserForm, PostForm, AboutAuthorForm


def index(request):

    post_objects = Articles.objects.all().order_by('id')[::-1]
...
def search_view(request):
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            articles_to_show = Articles(title=request.method['search'])
        else:
            articles_to_show = Articles(title=request.method['search'], article_permission='public')
